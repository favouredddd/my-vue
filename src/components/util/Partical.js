let Particle = function(container, params) {
  params = params || {};
  var options = {
    point: 100,
    minDis: 50,
  ...
  p.points = [];
  p.pageXY = {
    pageX: p.innerWidth / 2,
    pageY: p.innerHeight / 2
  };
  if (container[0] === '#') {
  ...
  p.init = function() {
    ...
    p.canvas.height = p.innerHeight-80;
    p.drawBackground();
    p.addOverlap();
    p.initDrawPoint();
    p.animation();
  ...
  p.addOverlap = function() {
    ...
    p.overlap.style.height = p.innerHeight-80 + 'px';
  ...
  p.initDrawPoint = function() {
    for (var i = 0; i < p.params.point; i += 1) {
  ...
  p.connect = function() {
    ...
    for (var i = 0; i < p.params.point; i += 1) {
      for (var j = 0; j < p.params.point; j += 1) {
        var p1 = p.points[i];
        var p2 = p.points[j];
  ...
  p.mouseEvent = function() {
    p.overlap.addEventListener('mousemove', function(e) {
      ...
      if (p.params.effect == 'zoom') {
        for (var t = 0; t < p.params.point; t += 1) {
